...
HTML 결과는 `npm run test:e2e:report`로 확인한다. 로컬 실패 trace와 임시 인증
파일은 Git에서 제외된다. CI에서는 인증 응답이 첨부물에 남지 않도록 trace·화면·
비디오 수집을 끄고, 스테이징 CI는 실패 시 HTML 보고서만 하루 보관한다.
...
- 학생 목록의 영구 캐시 제거, 이전 개인 과외 캐시 정리, 새로고침 후 서버 재조회
- 공개 학생 체크인의 서버 발급 QR 검증: 만료 생략·변조 및 비인증 토큰 직접 접근 차단
- 공용 QR과 만료 안내의 밝은/어두운 테마 및 동작 감소 설정
- SQL 역할 테스트 084~093 (QR 발급 권한, 만료·폐기, PIN 제한, 테스트 환경 차단,
  학생·직원 출결 직접 RPC/테이블 권한 포함)
