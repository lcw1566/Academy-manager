...
export default function MyPayrollCard({ role, myPayroll, myStaffProfile, enabled = false, onOpen }) {
  ...
  const payrollAmount = Math.max(0, Math.round(Number(myPayroll?.amount) || 0));
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-700">
            {enabled ? `${payrollAmount.toLocaleString()}원` : '파일럿 이후 제공 예정'}
          </p>
          <p className="text-xs text-gray-400 mt-0.5">
            {enabled ? '합성 근퇴 기록으로 만든 이번 달 급여 명세' : '급여 계산과 명세 기능을 준비하고 있어요.'}
          </p>
        </div>
